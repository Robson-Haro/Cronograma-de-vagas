...
export default function Home() {
  ...
  const summary = [
    "VAGAS TIMELINE — CRONOGRAMA DA VAGA",
    `Cargo: ${role || "Não informado"}`,
    `Grupo: ${config.label}`,
    `Time to Fill: ${config.timeToFill} dias corridos`,
    `Abertura: ${formatDate(parseDate(safeStartDate))}`,
    `Conclusão: ${formatDate(finalDate)}`,
    "",
    ...schedule.map((stage, index) => `${index + 1}. ${stage.name}: ${formatDate(stage.start)} a ${formatDate(stage.end)} (${stage.days} dia${stage.days === 1 ? "" : "s"})`),
    "",
  ...
  return (
    <main className="page-shell">
      ...
      <section className="actions-bar glass-card">
        <button className="primary-button" onClick={copySchedule}>{copied ? "✓ Copiado!" : "▣ Copiar cronograma"}</button>
